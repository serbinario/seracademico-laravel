...
$(document).on("click", "#btnEditPlanoAula", function () {
    ...
    if(tableConteudoPlanoAulaEditar) {
        loadEditTableConteudoProgramaticoPlanoAulaEditar(idPlanoAula).ajax.url("/index.php/seracademico/mestrado/planoEnsino/planoAula/gridConteudos/" + idPlanoAula).load();
    } else {
...
function loadFieldsPlanoAulaEditar()
{
    ...
    jQuery.ajax({
        type: 'POST',
        data: dados,
        url: '/index.php/seracademico/mestrado/planoEnsino/planoAula/getLoadFields',
        datatype: 'json'
...
function builderHtmlFieldsPlanoAulaEditar (dados) {
    ...
    jQuery.ajax({
        type: 'GET',
        url: '/index.php/seracademico/mestrado/planoEnsino/planoAula/edit/' + idPlanoAula,
        datatype: 'json'
    }).done(function (retorno) {
        if (retorno.success) {
...
$('#btnUpdatePlanoAula').click(function() {
    ...
    jQuery.ajax({
        type: 'POST',
        url: '/index.php/seracademico/mestrado/planoEnsino/planoAula/update/' + idPlanoAula,
        data: dados,
        datatype: 'json'
    }).done(function (retorno) {
        if(retorno.success) {
...
function loadFeldsConteudo() {
    ...
    jQuery.ajax({
        ...
        if(retorno) {
            ...
            // Percorrendo o array de cursos
            for (var i = 0; i < retorno['mestrado\\conteudoprogramatico'].length; i++) {
                htmlConteudo += "<option value='" + retorno['mestrado\\conteudoprogramatico'][i].id + "'>" + retorno['mestrado\\conteudoprogramatico'][i].nome + "</option>";
            }
