...
function loadFieldsDocumentos()
{
    ...
    var dados =  {
        'models' : [
            'Graduacao\\TipoDocumento|nivelDeGraduacao'
        ]
    };
    ...
    jQuery.ajax({
        type: 'POST',
        data: dados,
        url: '/index.php/seracademico/graduacao/aluno/turma/getLoadFields',
        datatype: 'json'
...
function builderHtmlFieldsDocumento(dados) {
    ...
    for (var i = 0; i < dados['graduacao\\tipodocumento'].length; i++) {
        htmlDocumento += "<option value='" + dados['graduacao\\tipodocumento'][i].id + "'>" + dados['graduacao\\tipodocumento'][i].nome + "</option>";
    }
...
$(document).on('click', '#btnGerarDocumento', function () {
    ...
    jQuery.ajax({
        type: 'GET',
        url: '/index.php/seracademico/graduacao/aluno/checkDocumento/'+ documentacao_id + "/" + idAluno,
        datatype: 'json'
    }).done(function (retorno) {
        // Verificando o retorno da requisição
        if(retorno.success) {
            // Executando o relatório e abrindo em outra aba
            window.open("/index.php/seracademico/graduacao/aluno/gerarDocumento/"
