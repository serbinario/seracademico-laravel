...
$('#btnAddConteudoPlanoAulaEditar').on( 'click', function () {
    ...
    jQuery.ajax({
        type: 'POST',
        url: '/index.php/seracademico/graduacao/planoEnsino/planoAula/attachConteudo/' + idPlanoAula,
...
$(document).on('click', '#btnDistroyPlanoAulaEditar', function () {
    ...
    jQuery.ajax({
        type: 'POST',
        url: '/index.php/seracademico/graduacao/planoEnsino/planoAula/detachConteudo/' + idPlanoAula,
        data: {'conteudos' : [idConteudo]},
        datatype: 'json'
    }).done(function (retorno) {
        if(retorno.success) {
