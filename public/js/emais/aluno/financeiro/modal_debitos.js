...
function loadTableDebitos (idAluno) {
    tableDebitos = $('#grid-debitos').DataTable({
        ...
        //bFilter: false,
        autoWidth: false,
        ajax: "/index.php/seracademico/emais/aluno/financeiro/gridDebitos/" + idAluno,
        columns: [
...
function loadTableCarnes (idAluno) {
    tableCarnes = $('#grid-carnes').DataTable({
        ...
        bFilter: false,
        autoWidth: false,
        ajax: "/index.php/seracademico/emais/aluno/financeiro/gridCarnes/" + idAluno,
        columns: [
            {data: 'gnet_carnet_id', name: 'fin_carnes.gnet_carnet_id'},
...
function runFinanceiro(idAluno) {
    if(tableDebitos) {
        loadTableDebitos(idAluno)
            .ajax
            .url("/index.php/seracademico/emais/aluno/financeiro/gridDebitos/" + idAluno)
            .load();
    } else {
        loadTableDebitos(idAluno);
    }
    ...
    if(tableCarnes) {
        loadTableCarnes(idAluno)
            .ajax
            .url("/index.php/seracademico/emais/aluno/financeiro/gridCarnes/" + idAluno)
            .load();
    } else {
        loadTableCarnes(idAluno);
    }
