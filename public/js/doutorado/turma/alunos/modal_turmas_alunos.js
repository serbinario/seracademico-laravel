...
function loadTableAlunos (idTurma) {
    tableAlunos = $('#alunos-grid').DataTable({
        ...
        bFilter: false,
        ajax: {
            url: "/index.php/seracademico/doutorado/turma/alunos/grid/" + idTurma,
            data: function (d) {
...
function runTableAlunos(idTurma) {
    ...
    if(tableCargaHoraria != null) {
        loadTableAlunos(idTurma).ajax.url( "/index.php/seracademico/doutorado/turma/alunos/grid/" + idTurma).load();
    } else {
...
$(document).on('change', '#disciplinaTurmaALunoSearch', function () {
    ...
    jQuery.ajax({
        type: 'GET',
        url: '/index.php/seracademico/doutorado/turma/getCalendariosByDisciplina/' + idTurma + '/' + idDisciplina,
        datatype: 'json'
    }).done(function (retorno) {
        // Verificando o retorno da requisição
        if(retorno) {
