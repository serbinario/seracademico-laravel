...
function loadTableDisciplina (idTurma) {
    tableDisciplina = $('#calendario-disciplina-grid').DataTable({
        ...
        bLengthChange: false,
       // bFilter: false,
        ajax: "/index.php/seracademico/doutorado/turma/calendario/grid/" + idTurma,
        columns: [
...
function runtableCargaHoraria () {
    tableCargaHoraria = $('#calendario-cargahoraria-grid').DataTable({
        ...
        ajax: "/index.php/seracademico/doutorado/turma/calendario/gridCalendario/" + idTurmaDisciplina,
        columns: [
...
$(document).on('click', '#calendario-disciplina-grid tbody tr', function () {
    ...
    if (tableDisciplina.rows().data().length > 0) {
        ...
        tableCargaHoraria.ajax.url( "/index.php/seracademico/doutorado/turma/calendario/gridCalendario/" + idTurmaDisciplina).load();
    }
});
...
function runTableDisciplina(idTurma) {
    ...
    loadTableDisciplina().ajax.url( "/index.php/seracademico/doutorado/turma/calendario/grid/" + idTurma).load();
    if(tableCargaHoraria != null) {
        tableCargaHoraria.ajax.url( "/index.php/seracademico/doutorado/turma/calendario/gridCalendario/" + 0).load();
    }
}