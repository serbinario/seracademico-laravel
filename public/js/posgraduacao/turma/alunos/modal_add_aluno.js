...
$(document).on("change", "#add_aluno_curso", function () {
    ...
    jQuery.ajax({
        type: 'GET',
        url: '/index.php/seracademico/posgraduacao/turma/alunos/getAlunosByCurso/' + idCurso + '/' + idTurma + '/' + idDisciplina,
        datatype: 'json'
    }).done(function (retorno) {
        // Verificando o retorno da requisição
        if(retorno.success) {
...
$(document).on("click", "#btnSaveAddAluno", function () {
    ...
    jQuery.ajax({
        type: 'POST',
        url: '/index.php/seracademico/posgraduacao/turma/alunos/attachAluno',
        data: dados,
        datatype: 'json'
    }).done(function (retorno) {
        if(retorno.success) {
