...
function loadTableDisciplinasPlanoEnsino (idTurma) {
    tableDisciplinasPlanoEnsino = $('#plano-ensino-disciplina-grid').DataTable({
        ...
        bLengthChange: false,
        bFilter: false,
        ajax: "/index.php/seracademico/posgraduacao/turma/planoEnsino/gridDisciplinas/" + idTurma,
        columns: [
...
function runTablePlanoEnsino(idTurma) {
    ...
    if (tableDisciplinasPlanoEnsino) {
        tableDisciplinasPlanoEnsino.ajax.url("/index.php/seracademico/posgraduacao/turma/planoEnsino/gridDisciplinas/" + idTurma).load();
    } else {
...
$(document).on('click', '#plano-ensino-disciplina-grid tbody tr td', function () {
    ...
    if($(this).text() != "" && $(this).index() != 0) {
        ...
        var dados =  {
            ...
            'models' : [
                'PosGraduacao\\PlanoEnsino|byDisciplinaAndCargaHoraria,' + disciplinaPlanoEnsinoId + ',' + cargaHoraria
            ]
        };
        ...
        jQuery.ajax({
            type: 'POST',
            data: dados,
            url: '/index.php/seracademico/posgraduacao/turma/planoEnsino/getLoadFields',
            datatype: 'json'
            ...
            if(retorno['posgraduacao\\planoensino'].length > 0) {
                // Percorrendo o array de disciplinacurriculo
                for(var i = 0; i < retorno['posgraduacao\\planoensino'].length; i++) {
                    // Criando as options
                    htmlPlanoEnsino += "<option value='" + retorno['posgraduacao\\planoensino'][i].id + "'>" + retorno['posgraduacao\\planoensino'][i].nome + "</option>";
                }
...
$(document).on('click', '#addPlanoEnsino', function () {
    ...
    jQuery.ajax({
        type: 'POST',
        data: dados,
        url: '/index.php/seracademico/posgraduacao/turma/planoEnsino/attachPlanoEnsino',
        datatype: 'json'
    }).done(function (retorno) {
