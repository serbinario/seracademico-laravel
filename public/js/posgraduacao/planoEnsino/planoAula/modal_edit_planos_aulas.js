...
$(document).on("click", "#btnEditPlanoAula", function () {
    ...
    if(tableConteudoPlanoAulaEditar) {
        loadEditTableConteudoProgramaticoPlanoAulaEditar(idPlanoAula).ajax.url("/index.php/seracademico/posgraduacao/planoEnsino/planoAula/gridConteudos/" + idPlanoAula).load();
    } else {
...
function loadFieldsPlanoAulaEditar()
{
    ...
    var dados =  {
        'models' : [
            'Professor|getValues',
            'PosGraduacao\\ConteudoProgramatico|byPlanoAula,' + idPlanoAula + ',' + idPlanoEnsino
        ]
    };
    ...
    jQuery.ajax({
        type: 'POST',
        data: dados,
        url: '/index.php/seracademico/posgraduacao/planoEnsino/planoAula/getLoadFields',
        datatype: 'json'
...
function builderHtmlFieldsPlanoAulaEditar (dados) {
    ...
    jQuery.ajax({
        type: 'GET',
        url: '/index.php/seracademico/posgraduacao/planoEnsino/planoAula/edit/' + idPlanoAula,
        datatype: 'json'
    }).done(function (retorno) {
        if (retorno.success) {
            ...
            // Percorrendo o array de cursos
            for (var i = 0; i < dados['posgraduacao\\conteudoprogramatico'].length; i++) {
                htmlConteudo += "<option value='" + dados['posgraduacao\\conteudoprogramatico'][i].id + "'>" + dados['posgraduacao\\conteudoprogramatico'][i].nome + "</option>";
            }
...
$('#btnUpdatePlanoAula').click(function() {
    ...
    jQuery.ajax({
        type: 'POST',
        url: '/index.php/seracademico/posgraduacao/planoEnsino/planoAula/update/' + idPlanoAula,
        data: dados,
        datatype: 'json'
    }).done(function (retorno) {
        if(retorno.success) {
...
function loadFeldsConteudo() {
    ...
    jQuery.ajax({
        ...
        if(retorno) {
            ...
            // Percorrendo o array de cursos
            for (var i = 0; i < retorno['posgraduacao\\conteudoprogramatico'].length; i++) {
                htmlConteudo += "<option value='" + retorno['posgraduacao\\conteudoprogramatico'][i].id + "'>" + retorno['posgraduacao\\conteudoprogramatico'][i].nome + "</option>";
            }
