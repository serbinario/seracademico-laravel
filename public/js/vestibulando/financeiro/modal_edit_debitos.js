$(document).on("click", "#btnEditarDebito", function () {
    ...
    idDebito = table.row($(this).parents('tr')).data().id;
...
function builderHtmlFieldsDebitoEditar (dados) {
    jQuery.ajax({
        type: 'GET',
        url: '/index.php/seracademico/vestibulando/financeiro/getDebito/' + idDebito,
        datatype: 'json'
    }).done(function (retorno) {
        if (retorno.success) {
            var htmlTaxa = "";
...
$('#btnUpdateDebito').click(function() {
    ...
    jQuery.ajax({
        type: 'POST',
        url: '/index.php/seracademico/vestibulando/financeiro/updateDebito/' + idDebito,
        data: dados,
        datatype: 'json'
    }).done(function (retorno) {
        if(retorno.success) {
