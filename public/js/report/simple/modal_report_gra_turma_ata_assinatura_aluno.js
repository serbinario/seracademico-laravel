...
function runSimpleReportGraTurmaAtaDeAssinaturaAluno()
{
    // Definindo os models
    var dados =  {
        'models' : [
    ...
    jQuery.ajax({
        ...
        if(retorno) {
            builderFilterGraTurmaAtaDeAssinaturaAluno(retorno);
        } else {
...
function builderFilterGraTurmaAtaDeAssinaturaAluno (dados) {
    //Limpando os campos
    $('#curso_gra_turma_ata_assinatura_aluno_id option').find('option').prop('selected', false);
    $('#turma_gra_turma_ata_assinatura_aluno_id option').find('option').prop('selected', false);
    $('#disciplina_gra_turma_ata_assinatura_aluno_id option').find('option').prop('selected', false);
    ...
    // carregando o html
    $("#curso_gra_turma_ata_assinatura_aluno_id option").remove();
    $("#curso_gra_turma_ata_assinatura_aluno_id").append(htmlCurso);
    $("#turma_gra_turma_ata_assinatura_aluno_id option").remove();
    $("#turma_gra_turma_ata_assinatura_aluno_id").append(htmlTurma);
    $("#disciplina_gra_turma_ata_assinatura_aluno_id option").remove();
    $("#disciplina_gra_turma_ata_assinatura_aluno_id").append(htmlDisciplina);

    // Abrindo o modal
    $("#modal-report-gra-turma-ata-assinatura-aluno").modal({show : true});
}

// Gerar o relatório
$('#btnBuilderReportGraTurmaAtaAssinaturaAluno').click(function() {
    // Recuperando o id do relatório selecionado
    var reportId = $('#report_id').val();
    var cursoId  = $('#curso_gra_turma_ata_assinatura_aluno_id').val();
    var turmaId  = $('#turma_gra_turma_ata_assinatura_aluno_id').val();
    var disciplinaId  = $('#disciplina_gra_turma_ata_assinatura_aluno_id').val();
