// carregando todos os camDou preenchidos
function runSimpleReportDouGeralAluno()
{
    // Definindo os models
    var dados =  {
        'models' : [
...
function builderFilterDouAlunoGeral (dados) {
    //Limpando os camdou
    $('#curso_dou_aluno_geral_id option').find('option').prop('selected', false);
    $('#turma_dou_aluno_geral_id option').find('option').prop('selected', false);
    ...
    // carregando o html
    $("#curso_dou_aluno_geral_id option").remove();
    $("#curso_dou_aluno_geral_id").append(htmlCurso);
    $("#turma_dou_aluno_geral_id option").remove();
    $("#turma_dou_aluno_geral_id").append(htmlTurma);

    // Abrindo o modal
    $("#modal-report-dou-aluno-geral").modal({show : true});
}

// Gerar o relatório
$('#btnBuilderReportDouAlunoGeral').click(function() {
    // Recuperando o id do relatório selecionado
    var reportId = $('#report_id').val();
    var cursoId  = $('#curso_dou_aluno_geral_id').val();
    var turmaId  = $('#turma_dou_aluno_geral_id').val();
