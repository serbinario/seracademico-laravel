...
function loadFieldsAddAluno()
{
    ...
    jQuery.ajax({
        type: 'POST',
        data: dados,
        url: '/index.php/seracademico/tecnico/turma/alunos/getLoadFields',
        datatype: 'json'
...
function builderHtmlFieldsAddAluno (dados) {
    ...
    for(var i = 0; i < dados['tecnico\\disciplina'].length; i++) {
        // Criando as options
        htmlDisciplina += "<option value='" + dados['tecnico\\disciplina'][i].id + "'>"  + dados['tecnico\\disciplina'][i].nome + "</option>";
    }
...
$(document).on("change", "#add_aluno_curso", function () {
    ...
    jQuery.ajax({
        type: 'GET',
        url: '/index.php/seracademico/tecnico/turma/alunos/getAlunosByCurso/' + idCurso + '/' + idTurma + '/' + idDisciplina,
        datatype: 'json'
    }).done(function (retorno) {
        // Verificando o retorno da requisição
        if(retorno.success) {
...
$(document).on("click", "#btnSaveAddAluno", function () {
    ...
    jQuery.ajax({
        type: 'POST',
        url: '/index.php/seracademico/tecnico/turma/alunos/attachAluno',
        data: dados,
        datatype: 'json'
    }).done(function (retorno) {
        if(retorno.success) {
