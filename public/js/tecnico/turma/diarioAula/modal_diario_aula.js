...
function loadTableDisciplinasDiarioAula (idTurma) {
    tableDisciplinasDiarioAula = $('#diario-aula-disciplina-grid').DataTable({
        ...
        bLengthChange: false,
        bFilter: false,
        ajax: "/index.php/seracademico/tecnico/turma/diarioAula/gridDisciplinas/" + idTurma,
        columns: [
...
function loadTableDiarioAula (idTurmaDisciplina) {
    tableDiarioAula = $('#diario-aula-grid').DataTable({
        ...
        bLengthChange: false,
        bFilter: false,
        ajax: "/index.php/seracademico/tecnico/turma/diarioAula/grid/" + idTurmaDisciplina,
        columns: [
            {
...
function runTableDiariosAulas(idTurma) {
    ...
    if (tableDisciplinasDiarioAula) { 
        tableDisciplinasDiarioAula.ajax.url("/index.php/seracademico/tecnico/turma/diarioAula/gridDisciplinas/" + idTurma).load();
    } else {
...
$(document).on('click', '#diario-aula-disciplina-grid tbody tr', function (event) {
    if (tableDisciplinasDiarioAula.rows().data().length > 0 && $(event.target).is("td")) {
        ...
        loadTableDiarioAula(idTurmaDisciplinaDiarioAula).ajax.url( "/index.php/seracademico/tecnico/turma/diarioAula/grid/" + idTurmaDisciplinaDiarioAula).load();
    }
});
...
$('#modal-diario-aula').on('hidden.bs.modal', function () {
    ...
    loadTableDiarioAula(idTurmaDisciplinaDiarioAula).ajax.url( "/index.php/seracademico/tecnico/turma/diarioAula/grid/" + 0).load();
});
