...
function loadTableCursoTurma(idAluno) {
    tableCursoTurma = $('#curso-turma-grid').DataTable({
        ...
        bFilter: false,
        //bPaginate: false,
        ajax: "/index.php/seracademico/tecnico/aluno/turma/grid/" + idAluno,
        columns: [
...
function loadTableSituacoes(idAlunoCurso) {
    tableSituacoes = $('#situacao-grid').DataTable({
        ...
        bFilter: false,
        //bPaginate: false,
        ajax: "/index.php/seracademico/tecnico/aluno/turma/gridSituacoes/" + idAlunoCurso,
        columns: [
...
$(document).on("click", "#curso-turma-grid tbody tr", function (event) {
    if (tableCursoTurma.rows().data().length > 0  && $(event.target).is("td")) {
        ...
        if(tableSituacoes) {
            loadTableSituacoes(idAlunoCurso).ajax.url("/index.php/seracademico/tecnico/aluno/turma/gridSituacoes/" + idAlunoCurso).load();
        } else {
...
function runCursoTurma(idAluno) {
    ...
    if(tableCursoTurma) {
        loadTableCursoTurma(idAluno).ajax.url("/index.php/seracademico/tecnico/aluno/turma/grid/" + idAluno).load();
    } else {
...
$(document).on('click', '#btnRemoverCurso', function () {
    ...
    jQuery.ajax({
        type: 'DELETE',
        url: '/index.php/seracademico/tecnico/aluno/turma/destroy/' + idAluno + '/' + id,
        datatype: 'json'
