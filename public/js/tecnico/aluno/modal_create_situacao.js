...
function loadFieldsSituacao()
{
    ...
    var dados =  {
        'models' : [
            'SituacaoAluno',
            'Tecnico\\Turma|posGraduacaoByAlunoCurso,' + idAlunoCurso
        ]
    };
...
function loadTurmaOrigem()
{
    ...
    jQuery.ajax({
        type: 'GET',
        url: '/index.php/seracademico/tecnico/aluno/turma/getTurmaOrigem/' + idAlunoCurso,
        datatype: 'json'
    }).done(function (retorno) {
        // Verificando o retorno da requisição
        if(retorno) {
...
$(document).on("click", "#btnSalvarSituacao", function () {
    ...
    jQuery.ajax({
        type: 'POST',
        url: '/index.php/seracademico/tecnico/aluno/turma/storeSituacao',
        data: dados,
        datatype: 'json'
    }).done(function (retorno) {
        if(retorno.success) {
...
$(document).on('click', '#btnRemoverSituacao', function () {
    ...
    jQuery.ajax({
        type: 'DELETE',
        url: '/index.php/seracademico/tecnico/aluno/turma/destroySituacao/' + idAlunoSituacao,
        datatype: 'json'
    }).done(function (retorno) {
        if(retorno.success) {
            tableSituacoes.ajax.reload();
