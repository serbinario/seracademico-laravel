        ...
        <div class="fileinput fileinput-new" data-provides="fileinput">
            <div class="fileinput-preview thumbnail" data-trigger="fileinput" id="captura" style="width: 135px; height: 115px;">
                @if (isset($aluno) && $aluno->path_image != null)
                    <div id="midias">
                        <img id="logo" src="{{route('seracademico.doutorado.aluno.getImgAluno', ['id' => $aluno->id])}}"  alt="Foto" height="120" width="100"/><br/>
                ...
                <div class="row">
                    <div class="form-group col-md-4">
                        <div class="fg-line">
                            ...
                            @else
                                {!! Form::select('curso_id', (['' => 'Selecione um Curso'] + $loadFields['doutorado\\curso']->toArray()), null, array('class' => 'form-control', 'id' => 'curso_id')) !!}
                            @endif
                        </div>
                    </div>
...
@section('javascript')
    ...
    {{--Regras de validação--}}
    {{--<script type="text/javascript" src="{{ asset('/js/validacoes/doutorado/aluno.js')  }}"></script>--}}
    <script type="text/javascript">
        ...
        $(document).on('click', '#btnSearchCpf', function () {
            ...
            jQuery.ajax({
                type: 'GET',
                url: '{{ route('seracademico.doutorado.aluno.search')  }}',
        ...
        **/
        function getTurmasByCurso(cursoId)
        {
            ...
            jQuery.ajax({
                type: 'GET',
                url: '/index.php/seracademico/doutorado/turma/getAllByCurso/' + cursoId,
                datatype: 'json',
                headers: {
                    'X-CSRF-TOKEN' : '{{  csrf_token() }}'
                },
        ...
        function getSedeByCurso(cursoId)
        {
            ...
            jQuery.ajax({
                type: 'GET',
                url: '/index.php/seracademico/doutorado/turma/getSedeByCurso/' + cursoId,
                datatype: 'json',
                headers: {
                    'X-CSRF-TOKEN' : '{{  csrf_token() }}'
                }
        ...
        function getTurmaBySede(sedeId, cursoId)
        {
            ...
            jQuery.ajax({
                type: 'GET',
                url: '/index.php/seracademico/doutorado/turma/getTurmaBySede/' + sedeId + '/' + cursoId,
                datatype: 'json',
                headers: {
                    'X-CSRF-TOKEN' : '{{  csrf_token() }}'
                }
